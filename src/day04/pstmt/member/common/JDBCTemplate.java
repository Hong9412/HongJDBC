...
public class JDBCTemplate {
	// 연결작업은 시간이 걸리는 작업임
	// 자주하면 좋지 않아요.
	// 한번 작업하고 만들어 놓은 것을 사용하기 위해서 디자인 패턴 적용
	// 디자인 패턴이란?
	// 각기 다른 소프트웨어 모듈이나 기능을 가진 응용SW를
	// 개발할 때 공통되는 설계 문제를 해결하기 위하여 사용되는 패턴임
	// 1. 생성패턴 : 싱글톤 패턴, 추상팩토리, 팩토리 메서드
	// 2. 구조패턴 : 컴포지트, 데코레이트, ...
	// 3. 행위패턴 : 옵저버, 스테이트, 전력, 템플릿 메서드, ...
	...
	public static JDBCTemplate getInstance() {
		if (instance == null) { // 객체가 만들어져 있는지 체크 후
			instance = new JDBCTemplate(); // 없으면 만들어 줌
		}
		return instance; // 만들어 놓은 것을 사용할 수 있도록 리턴해 줌
	}
